...
## Результаты экспериментов по дистилляции Whisper large-v3

В данном разделе представлены результаты экспериментов по дистилляции модели `openai/whisper-large-v3` для уменьшения ее размера и ускорения инференса. Были проведены эксперименты с созданием студенческих моделей с 2 и 8 слоями декодера. 

### Сводная таблица результатов

| Модель                 | Сл. декодера | Вес (MB, fp32) | GPU-latency¹ (20 × 30 s) | Ускорение | WER dev-clean (100)         |
|------------------------|----------------|------------------|--------------------------|-----------|-----------------------------|
| openai/whisper-large-v3| 32             | 6174             | 3895 ms                  | 1×        | 8.74%                       |
| large-v3 → 2-layer     | 2              | 2900             | 240 ms                  | 16×       | 393% (почти пустые строки)  |
| large-v3 → 8-layer     | 8              | 3655             | 1547 ms                  | 2.5×      | 96.57%                      |

*¹ Латентность измерена на GPU для батча из 20 аудиофрагментов по 30 секунд каждый.*

### Краткий обзор эксперимента с 8-слойной моделью (из `distill.ipynb`)

Студенческая модель с 8 слоями декодера была создана на основе `openai/whisper-large-v3`.

*   **Инициализация**:
    *   Конфигурация декодера изменена на 8 слоев (`decoder_layers = 8`).
    *   Веса сверточных слоев (`conv1`, `conv2`) энкодера были скопированы из модели-учителя.
    *   4 слоя декодера студента были инициализированы весами соответствующих слоев учителя (слои `[0,1,6,7]` студента из `[0,1,-2,-1]` учителя).
*   **Обучение**:
    *   Применялась дистилляция знаний с комбинированной функцией потерь: CrossEntropy (CE) и KL Divergence (KD loss, `alpha_kd=0.3`).
    *   Энкодер студента был заморожен в течение первой эпохи обучения, затем разморожен для совместного дообучения с декодером.
    *   Модель обучалась в течение 6 эпох (`num_train_epochs=6`) на датасете LibriSpeech (`train-clean-100`).
    *   Использовался оптимизатор AdamW с косинусным планировщиком скорости обучения (`lr_scheduler_type="cosine"`, `learning_rate=1e-4`).

### Анализ результатов

**Сравнение студенческих моделей:**

*   **2-слойная модель:**
    *   **Скорость/память:** Максимальное ускорение (≈16×) и экономия VRAM (вес ~2900 MB).
    *   **Качество:** Модель практически не выучила язык (WER 393%), генерирует почти пустые строки.
*   **8-слойная модель:**
    *   **Скорость/память:** Ускорение в ≈2.5 раза по сравнению с учителем, вес модели уменьшен на ~40% (3655 MB).
    *   **Качество:** Качество распознавания значительно лучше, чем у 2-слойной модели, но все еще очень далеко от модели-учителя (WER 96.57%).

**Возможные причины высокого WER у 8-слойной студенческой модели:**

*   **Ограниченное обучение на GT-текстах:** Использование Ground Truth текстов началось только с текущего эксперимента, что означает, что модель «не слушала» достаточное количество аудио для качественного обучения с самого начала.
*   **Короткий период дообучения энкодера:** Энкодер был разморожен только после первой эпохи, а общее количество эпох обучения составило всего 6. Этого может быть недостаточно для полной адаптации энкодера.
*   **Сокращенное количество параметров декодера:** В 8-слойной модели количество параметров декодера значительно меньше (8 из 32 слоев оригинальной модели), что объективно затрудняет выучивание полноценной языковой модели (LM) на уровне учителя.

### Перспективы улучшения качества (дополнительные 8–10 часов работы)

| Шаг                          | Изменения                                            | Ожидаемое улучшение WER / Эффект        |
|------------------------------|------------------------------------------------------|-----------------------------------------|
| Увеличение эпох обучения      | `num_train_epochs = 10–12`                           | + ~2–3 п.п. к WER                       |
| Клиппинг градиента и LR      | `learning_rate` → 5e-5, `max_grad_norm = 1`          | Более стабильное обучение               |
| Отложенная дистилляция (KD)  | Первые 3 эпохи — только CE, затем CE + KD            | Потенциально более быстрая сходимость    |
| Увеличение слоев декодера    | `decoder_layers = 12` (ориентировочный вес ~4.8 GB) | Ожидаемый WER ≈ 20–25%                  |
