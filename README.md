...
| Модель                     | Метод                       | Размер весов (прибл. MB)        | Время инференса (CPU, ms) | Время инференса (GPU, ms) | Использование RAM (MB) (пиковое процесса) | Использование VRAM (MB) | Качество (WER / CER)        |
| :------------------------- | :-------------------------- | :------------------------------ | :------------------------- | :------------------------- | :-------------------------------------- | :---------------------- | :-------------------------- |
| `openai/whisper-large-v3`  | FP16         | 2940.31                         | 4504.25                        | 540.48                     | 40564.33                                | 3203.10                 | 0.1468 / 0.0490             |
| `openai/whisper-large-v3`  | INT8 Dynamic (CPU)          | 1752.12                         | 2671.29                    | N/A                        | 41182.55                                | N/A                     | 0.1519 / 0.0500             |
| `openai/whisper-large-v3`  | Pruning FP16 | 2481.61 (эфф.) / 2940.31 (общ.) | N/A                        | 538.42                     | 70791.42                                | 6151.46                 | 0.1215 / 0.0497             |
| `openai/whisper-large-v3`  | GPTQ                        | *Не применимо (см. исследование)* | -                          | -                          | -                                       | -                       | *N/A*                       |
| `openai/whisper-large-v3`  | AWQ                         | *Не применимо (см. исследование)* | -                          | -                          | -                                       | -                       | *N/A*                       |

*(N/A - не применимо или не измерялось для данной конфигурации)*

### Исследование по неприменимости GPTQ и AWQ к openai/whisper-large-v3 с использованием стандартных библиотек (optimum, auto-gptq, autoawq):

Применение методов квантизации GPTQ и AWQ к модели openai/whisper-large-v3 с использованием текущих версий популярных библиотек (optimum, auto-gptq и autoawq) на данный момент сопряжено со значительными трудностями или невозможно "из коробки".

Для GPTQ:

Инструменты, такие как GPTQQuantizer из библиотеки optimum, в первую очередь ориентированы на текстовые (token-based) модели Transformer (например, BERT, GPT). Они ожидают на вход тензоры input_ids и часто автоматически генерируют attention_mask.

Модель Whisper (класс AutoModelForSpeechSeq2Seq) принимает на вход аудио-признаки (input_features) с другой размерностью (например, [batch, n_mels, n_frames]), что приводит к ошибкам индексации и несовместимости с внутренней логикой существующих GPTQ-квантизаторов, не рассчитанных на обработку speech-входов.

Адаптация GPTQ для Whisper потребовала бы существенной модификации кода квантайзера для корректной обработки аудио-признаков, отключения генерации attention_mask для этих признаков и переопределения процедур вычисления градиента/Гессиана под специфику архитектуры и размерности входов. На момент исследования, готовых публичных решений или патчей, реализующих такую поддержку в основных библиотеках, не обнаружено.

Для AWQ:

Существующие реализации AWQ (например, библиотека autoawq и ее интеграции) также сфокусированы на LLM с преимущественно линейными слоями (архитектуры Llama, OPT, Falcon, Mistral и т.д.).

Архитектура Whisper включает сверточные слои (Conv1d/Conv2d) в начале и имеет специфику энкодер-декодерной Seq2Seq модели. Часть линейных слоев может иметь размерности, не поддерживаемые специализированными низкоуровневыми ядрами (GEMM/ExLlama), используемыми в AWQ для 4-битных вычислений.

На текущий момент, 4-битные ядра, необходимые для эффективного инференса AWQ, не реализованы для сверточных слоев или специфических компонентов Seq2Seq моделей аудио.

Альтернативы для экстремальной квантизации Whisper: динамическая INT8/INT4 квантизация через torch.quantization, использование bitsandbytes для экономии VRAM, или экспорт в форматы ONNX/TensorRT/GGUF с последующей квантизацией в этих средах.

### N/A (Not Applicable / Не применимо):

Время инференса (CPU, ms) для FP16 (GPU Baseline) и Pruning L1Unstr. (20%) FP16: Эти методы в данном контексте оценивались на GPU с использованием типа данных FP16. Измерение их производительности на CPU в FP16 не проводилось, так как FP16 на CPU часто не дает выигрыша в скорости по сравнению с FP32 и не является типичным сценарием использования

Время инференса (GPU, ms) для INT8 Dynamic (CPU): Динамическая INT8 квантизация в torch.quantization предназначена и оптимизирована в первую очередь для ускорения на CPU. Запуск этой конкретной квантованной модели на GPU не является стандартным сценарием и не измерялся.

Использование VRAM (MB) для INT8 Dynamic (CPU): Так как этот метод оценивался на CPU, использование видеопамяти (VRAM) нерелевантно.

Качество (WER / CER) для GPTQ и AWQ: Поскольку сами методы не удалось применить к модели для получения квантованной версии, оценка качества для них не проводилась.

### Результаты квантизации языковых моделей

Поскольку методы GPTQ и AWQ хорошо применимы к стандартным архитектурам больших языковых моделей (LLM), для них ведутся отдельные замеры. В этой секции будут представлены результаты для моделей типа `facebook/opt-125m`

| Модель              | Метод        | Размер весов (MB) | Время инференса (GPU, ms, 50 токенов) | VRAM (Perplexity, MB) | VRAM (Inference, MB) | Качество (Perplexity GPU) |
| :------------------ | :----------- | :---------------- | :------------------------------------- | :-------------------- | :------------------- | :------------------------ |
| `facebook/opt-125m` | FP16         | 238.90            | 291.26                                 | 1579.18               | 574.84               | 27.62                     |
| `facebook/opt-125m` | GPTQ (4-bit) | 123.91            | 320.15                                 | 1691.36               | 367.17               | 30.93                     |
| `facebook/opt-125m` | AWQ (4-bit)  | *N/A*              | *N/A*                                   | *N/A*                 | *N/A*                 | *N/A*                       |
|                     |              |                   |                                        |                       |                      |                           |

*(N/A - не применимо или не измерялось для данной конфигурации)*

После попытки применить AWQ к `facebook/opt-125m` выяснилось, что текущая версия autoawq поддерживает только модели с Rotary Positional Embeddings (семейства Llama, Mistral, Gemma, Qwen2 и др.). OPT использует обычные learned-position embeddings, поэтому квантайзер падает на попытке обращения к `model.rotary_emb`. В результате для этой модели AWQ-квантизацию выполнить не удалось.
